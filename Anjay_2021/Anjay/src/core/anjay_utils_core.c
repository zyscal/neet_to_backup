...
_anjay_transport_info_by_uri_scheme(const char *uri_or_scheme) {
    ...
    /////////////ZYS
    // printf("sizeof transports : %d\n\n", (int)(AVS_ARRAY_SIZE(TRANSPORTS)));
    // for(int i = 0; i < AVS_ARRAY_SIZE(TRANSPORTS); ++i)
    // {
    //     printf("%s\n", TRANSPORTS[i].uri_scheme);
    // }

    for (size_t i = 0; i < AVS_ARRAY_SIZE(TRANSPORTS); ++i) {
        size_t scheme_size = strlen(TRANSPORTS[i].uri_scheme);
        int result = avs_strncasecmp(uri_or_scheme, TRANSPORTS[i].uri_scheme,
                            scheme_size);
        anjay_log(WARNING, _("result : ") "%d", result);

        if (result == 0 && (uri_or_scheme[scheme_size] == '\0'
                    || uri_or_scheme[scheme_size] == ':')) {
            return &TRANSPORTS[i];
        }
    }
    printf("wqwqwqwqwqwqw\n\n\n");
    anjay_log(WARNING, _("unsupported URI schem     e: ") "%s", uri_or_scheme);
    return NULL;
}
